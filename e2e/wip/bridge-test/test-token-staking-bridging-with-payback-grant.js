...
module.exports = async function () {
  try {
    ...
    const grantData = web3.eth.abi.encodeParameters(
      ...
      [owner, grantee, 10, 0, 0, true, stakingPolicy.address]
    );
    ...
    await tokenGrant.undelegate(operator, { from: grantee });
    await new Promise((r) => setTimeout(r, 5000));
    await tokenGrant.recoverStake(operator, { from: grantee });

    expect(await tokenGrant.availableToStake(grantID)).to.eq.BN(grantAmount);

    await tokenGrant.withdraw(grantID, { from: grantee });
