...
async function run() {
  try {
    ...
    const genesisBlockHash = (await bitcoinRpc.getblockhashAsync(genesisBlock))
      .result
    const genesisBlockHeader = (
      await bitcoinRpc.getblockheaderAsync(genesisBlockHash, false)
    ).result

    const epochStartBlock = genesisBlock - (genesisBlock % 2016)
    const epochStartBlockHash = (
      await bitcoinRpc.getblockhashAsync(epochStartBlock)
    ).result
